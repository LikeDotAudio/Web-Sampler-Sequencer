...
// Human-readable release stamp shown in the footer: VYYYYMMDD.HHMM, in UTC so
// it means the same thing wherever it was built. Distinct from OA_BUILD_STAMP,
// which is a content hash — --check compares only that, so an unchanged source
// tree stays "up to date" and CI does not churn a new dist on every run.
const d = new Date();
